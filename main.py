...
async def create_transaction(tx: TransactionCreate, db: Session = Depends(get_db)):
    ...
    is_flagged = False
    if tx.amount > 1000 or is_new_recipient:
        is_flagged = True
    db_tx = Transaction(
        user_id=tx.user_id,
        amount=tx.amount,
...
async def check_sms(sms: SMSCheck, db: Session = Depends(get_db)):
    ...
    is_phishing = any(keyword in sms_text for keyword in scam_keywords)
    if is_phishing:
        message = "Warning: Possible scam SMS. Do not respond or send money."
        send_ussd(user.phone, message)
